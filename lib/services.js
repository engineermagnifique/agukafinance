...
const FALLBACK_SERVICES = FALLBACK_SERVICES_EN.map((service) => {
  const rw = SERVICE_TRANSLATIONS_RW[service.title];
  return {
    ...service,
    titleRw: rw?.title || null,
    descriptionRw: rw?.description || null,
    ctaLabelRw: rw?.ctaLabel || null,
    accordionItemsRw: rw?.accordionItems || [],
  };
});

// Adds a `display` object with the copy to show in the visitor's language.
// The English fields stay untouched because they double as the values sent
// with consultation requests (see components/ui/consultation-form.js).
export function localizeService(service, locale) {
  const useRw = locale === "rw";
  return {
    ...service,
    display: {
      title: (useRw && service.titleRw) || service.title,
      description: (useRw && service.descriptionRw) || service.description,
      ctaLabel: (useRw && service.ctaLabelRw) || service.ctaLabel,
      accordionItems: useRw && service.accordionItemsRw.length ? service.accordionItemsRw : service.accordionItems,
    },
  };
}
  ...
  const [result] = await pool.execute(
    `INSERT INTO services
      (title, description, icon, cta_type, cta_label, cta_href, accordion_items, is_active, sort_order,
       title_rw, description_rw, cta_label_rw, accordion_items_rw)
     VALUES
      (:title, :description, :icon, :ctaType, :ctaLabel, :ctaHref, :accordionItems, :isActive, :sortOrder,
       :titleRw, :descriptionRw, :ctaLabelRw, :accordionItemsRw)`,
    toRow(data)
  );
...
export async function updateService(id, data) {
  ...
  await pool.execute(
    `UPDATE services SET
      ...
      cta_label_rw = :ctaLabelRw,
      accordion_items_rw = :accordionItemsRw
     WHERE id = :id`,
    { ...toRow(data), id }
  );
}
...
function toJsonList(items) {
  return Array.isArray(items) && items.length > 0 ? JSON.stringify(items) : null;
}
...
function toRow(data) {
  return {
    ...
    accordionItems: toJsonList(data.accordionItems),
    isActive: data.isActive ? 1 : 0,
    sortOrder: Number.isFinite(data.sortOrder) ? data.sortOrder : 0,
    titleRw: data.titleRw || null,
    descriptionRw: data.descriptionRw || null,
    ctaLabelRw: data.ctaLabelRw || null,
    accordionItemsRw: toJsonList(data.accordionItemsRw),
  };
}
...
function parseRow(row) {
  return {
    ...
    titleRw: row.title_rw || null,
    descriptionRw: row.description_rw || null,
    ctaLabelRw: row.cta_label_rw || null,
    accordionItemsRw: row.accordion_items_rw ? JSON.parse(row.accordion_items_rw) : [],
